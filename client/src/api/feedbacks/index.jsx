...
export const getAllFeedbacksForVeterinarian = async ({
    ...
    return axiosInstance.get(
      `${import.meta.env.VITE_API_URL}/feedbacks/veterinarians/?${params.toString()}`,
    );
};
...
export const updateFeedback = async (id, { rating, comment, verify }) => {
  try {
    const response = await axiosInstance.patch(`/feedbacks/${id}`, {
      rating, comment, verify
    }, {
      headers: {
    ...
    return { success: false, message: "Une erreur est survenue lors de la mise à jour du commentaire" };
  }
};
