      ...
      <div className="mx-auto mt-32 max-w-7xl px-6 sm:mt-48 lg:px-8">
        <div className="mx-auto max-w-2xl lg:mx-0">
          <h2 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            {t("components.organisms.vetenarian.teamSectionComponent.h2")}
          </h2>
        </div>
        <ul
          role="list"
        ...
        >
          {teams.map((person) => (
            <li key={person.uuid}>
              {admin && (
                <TrashIcon
                  className="w-4 font-bold text-red-500 hover:text-black hover:scale-110 duration-75 ease-in cursor-pointer"
                  onClick={() => deleteVeterinarianByAdmin(person.uuid)}
                />
              )}

              <LinkBase
                to={`/veterinaire/${person.uuid}`}
              ...
              >
                <img
                  ...
                  alt=""
                />
                <h3 className="mt-6 text-base font-semibold leading-7 tracking-tight text-gray-900 uppercase">
                  DR. {person.name}
                </h3>
