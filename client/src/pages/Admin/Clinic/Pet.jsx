      ...
      getAllPets()
        ...
        .then(([clinicResponse, petsResponse]) => {
          ...
          setClinicInfo(prev => ({
            ...prev,
            clinic: clinicData,
            teams: clinicData.veterinarians.map(({firstname, lastname, specialties, uuid}) => ({
                        ...
                        <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
                          ...
                          <div
                            ...
                            {pets["hydra:member"].map((pet) => (
                                <Link key={pet.id} className="group" to={`/administration/animal/${pet.uuid}`}>
                                  <div
                                  ...
                                  <h3 className="mt-4 text-sm text-gray-700 uppercase">{pet.userID.firstname} {pet.userID.lastname}</h3>
                                  <p className="mt-1 text-lg font-medium text-gray-900">{pet.name}</p>
                                </Link>
                            ))}
                          </div>
                        </div>
